...
class App:
    ...
    def exit_open_menu(self, opt):
        ...
        close_menu.grid(row=1, column=1, padx=5, pady=5)

        go_to_menu = Button(opt, text="Go To Menu", command=lambda: [self.win.deiconify(), opt.withdraw()])
        go_to_menu.grid(row=1, column=2, padx=5, pady=5)
